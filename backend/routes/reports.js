...
router.get('/data', authenticate, authorize('admin'), logActivity, async (req, res) => {
  try {
...
// Export to Excel
router.get('/export/excel', authenticate, authorize('admin'), logActivity, async (req, res) => {
  try {
    const { category, status, dateFrom, dateTo } = req.query;
    ...
    tickets.forEach(ticket => {
      worksheet.addRow({
        ...
        technician: ticket.assignedTechnician?.fullName || '-',
        description: ticket.description || '-'
      });
    });

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', 'attachment; filename=laporan-tiket.xlsx');
...
// Export to PDF
router.get('/export/pdf', authenticate, authorize('admin'), logActivity, async (req, res) => {
  try {
    const { category, status, dateFrom, dateTo } = req.query;
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=laporan-tiket.pdf');

    doc.pipe(res);

    // Header
    doc.fontSize(20).text('Laporan Tiket', { align: 'center' });
    doc.moveDown();

    // Table header
    const tableTop = doc.y;
    const rowHeight = 20;
    const colWidths = [50, 80, 70, 70, 70, 70, 70, 120];
    const headers = ['Kategori', 'Nomor Tiket', 'Tanggal Masuk', 'Tanggal Update', 'Pelapor', 'Asal Unit', 'Teknisi', 'Deskripsi Masalah'];

    let y = tableTop;
    doc.fontSize(9).font('Helvetica-Bold');
    headers.forEach((header, i) => {
      doc.text(header, 50 + colWidths.slice(0, i).reduce((a, b) => a + b, 0), y, { width: colWidths[i], ellipsis: true });
    });

    y += rowHeight;
    const totalWidth = colWidths.reduce((a, b) => a + b, 0);
    doc.moveTo(50, y).lineTo(50 + totalWidth, y).stroke();
    ...
    tickets.forEach((ticket, index) => {
      ...
      row.forEach((cell, i) => {
        const cellText = cell || '-';
        doc.text(cellText, 50 + colWidths.slice(0, i).reduce((a, b) => a + b, 0), y, { width: colWidths[i], ellipsis: true });
      });
