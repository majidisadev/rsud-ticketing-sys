...
const { Ticket, ProblemType } = require("../models");
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // All-time tickets by problem type (for pie chart)
      const problemTypeCounts = await Ticket.findAll({
        where: {
          isActive: true,
        },
        include: [
        ...
        attributes: [
          [Sequelize.col("problemType.name"), "name"],
          [Sequelize.fn("COUNT", Sequelize.col("Ticket.id")), "count"],
        ],
        group: [Sequelize.col("problemType.name")],
        raw: true,
      });

      const allTimeByProblemType = problemTypeCounts
        .map((item) => ({
          name: item.name || "Belum ada kategori",
  ...
  async (req, res) => {
    try {
      ...
      while (current.isSameOrBefore(end, "month")) {
        ...
        result.push(
          monthMap[monthKey] || { month: monthName, SIMRS: 0, IPSRS: 0 },
        );
        current.add(1, "month");
      }
